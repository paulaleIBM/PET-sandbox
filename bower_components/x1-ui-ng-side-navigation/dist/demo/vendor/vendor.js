...
angular.module("x1.ui.demo-generator",["ui.bootstrap.buttons"]);
angular.module("x1.ui.demo-generator").directive("x1DemoGenerator",["x1.ui.demo-generator.BowerRegistry",function(BowerRegistry){"use strict";return{restrict:"E",transclude:!0,templateUrl:"demo-generator/demo-generator.html",scope:{repoName:"@?",docSrc:"="},link:function($scope){$scope.demoTemplate="demo",$scope.repoName&&BowerRegistry.getData().then(function(data){$scope.repoUrl=data[$scope.repoName].url})}}}]);
angular.module("x1.ui.demo-generator").factory("x1.ui.demo-generator.BowerRegistry",["$q",function($q){"use strict";function getData(){return data.then(function(data){data.data.forEach(function(repo){repo.url=repo.url.replace("-dist/bower-","/","gi"),bowerStore[repo.name]=repo}),deferred.resolve(bowerStore)})["catch"](function(){deferred.reject({})}),deferred.promise}function setData(_data){data=_data}var data,deferred=$q.defer(),bowerStore={};return{getData:getData,setData:setData}}]);
angular.module("x1.ui.demo-generator").run(["$http","x1.ui.demo-generator.BowerRegistry",function($http,BowerRegistry){"use strict";BowerRegistry.setData($http.get("http://x1showcase.emmlabs.ibm.com:5678/packages"))}]);
!function(module){try{module=angular.module("x1.ui.demo-generator")}catch(e){module=angular.module("x1.ui.demo-generator",[])}module.run(["$templateCache",function($templateCache){$templateCache.put("demo-generator/demo-generator.html",'<div class="x1-demo-generator-btn-group btn-group" role="group"><button type="button" class="btn btn-secondary" ng-model="demoTemplate" btn-radio="\'demo\'" ng-class="{\'active\':demoTemplate===\'demo\'}">Demo</button> <button type="button" class="btn btn-secondary" ng-model="demoTemplate" btn-radio="\'doc\'" ng-class="{\'active\':demoTemplate===\'doc\'}">Documentation</button></div><div class="x1-demo-generator-content row"><article ng-if="demoTemplate===\'demo\'" ng-transclude="" class="col-md-9"></article><article ng-if="demoTemplate===\'doc\'" ng-include="docSrc" class="col-md-9"></article><aside class="col-md-3"><a ng-if="repoUrl" ng-href="{{repoUrl}}" target="_blank" class="x1-demo-generator-btn btn btn-primary">View the code</a></aside></div>')}])}();
